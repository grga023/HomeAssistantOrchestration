...
dokumentima link:01-overview.adoc[01]–link:11-cpu-i-dijagnostika.adoc[11].
...
Pored funkcionalnog dela, projekat sadrži i *mernu studiju* (dokumenti 01–11)
...
* *ESP-IDF 5.5.0* (čist C), bez Arduino sloja
...
[NOTE]
====
*Izvedeni build (v3.0.0)* na čvoru B *nema* grejač ni vlažnost — samo *4 simulirana
temperaturna senzora* (+ LCD i diag). Tabele iznad prikazuju *prvobitni per-ploča
obrazac* (DHT22 + grejač); stvarna odstupanja su dokumentovana u
link:07-okruzenje-i-izvodjenje.adoc[07] i link:10-realni-podaci-uredjaja.adoc[10].
====
...
[NOTE]
====
*Praktično ograničenje (izmereno):* OTA sa GitHub Releases trenutno *pada* jer
`.../releases/latest/download/<asset>` radi višestruki 302 redirect ka potpisanom
`githubusercontent.com` URL-u, koji on-device `esp_https_ota` / `fetch_manifest` *ne
prati* (`ESP_FAIL` odnosno `manifest: empty body`). Zato se za sada firmver fleš-uje
preko USB-a; puni nalaz i rešenje su u link:10-realni-podaci-uredjaja.adoc[10].
Popravka (pratiti `Location` zaglavlje) je pravac daljeg rada.
====
...
Celokupna dokumentacija (00–11) se sklapa u jedan `.docx` jednom komandom — vidi
